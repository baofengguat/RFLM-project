...
class Generator(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.main_module = nn.Sequential(
            # Z latent vector 100
            nn.ConvTranspose2d(in_channels=100, out_channels=512, kernel_size=7, stride=1, padding=0),
            ...
            # output of main module --> Image (Cx224x224)
...
class Discriminator(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.main_module = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True))
            # output of main module --> State (512x7x7)
...
class WGAN_GP(object):
    def __init__(self, args):
        ...
        summary(self.G, (100, 1, 1))
        summary(self.D, (args.channels, 224, 224))
    ...
    def train(self, train_loader):
        ...
        for g_iter in range(self.generator_iters):
            ...
            for d_iter in range(self.critic_iter):
                ...
                d_loss_real = d_loss_real.mean() # original d_loss_real
                ...
                d_loss_fake = d_loss_fake.mean() # original d_loss_fake
            ...
            if (g_iter) % SAVE_PER_TIMES == 0:
                ...
                grid = utils.make_grid(samples, nrow=1)
    ...
    def evaluate(self, test_loader, D_model_path, G_model_path,save_path,number):
        self.load_model(D_model_path, G_model_path)
        for i in range(number):
            z = self.get_torch_variable(torch.randn(1, 100, 1, 1))
            samples = self.G(z)
            samples = samples.mul(0.4).add(0.4)
            samples = samples.data.cpu()
            grid = utils.make_grid(samples, nrow=1)
            noise = np.random.randn(grid.size(0),grid.size(1),grid.size(2))
            grid = grid+noise

            utils.save_image(grid, os.path.join(save_path,'dgan_1_%simage.png'%i))
            print("Grid of 1x1 images saved to 'dgan_model_image.png'.")
    ...
    def real_images(self, images, number_of_images):
        if (self.C == 3):
            return self.to_np(images.view(-1, self.C, 224, 224)[:self.number_of_images])
        else:
            return self.to_np(images.view(-1, 224, 224)[:self.number_of_images])
    ...
    def generate_img(self, z, number_of_images):
        ...
        for sample in samples:
            if self.C == 3:
                generated_images.append(sample.reshape(self.C, 224, 224))
            else:
                generated_images.append(sample.reshape(224, 224))
    ...
    def generate_latent_walk(self, number):
        ...
        for i in range(1, number_int + 1):
            ...
            images.append(fake_im.view(self.C,224,224).data.cpu())
